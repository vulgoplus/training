export function getCities() {
  const obj = city_district
  let cities = Object.keys(obj).map(key => {
    return obj[key].name
  })
  return cities
}
...
export function getDistricts(city) {
  ...
  Object.keys(obj).find(key => {
    if (obj[key].name === city) {
      const obj_district = obj[key].districts
      const data = Object.keys(obj_district).map(key_district => {
        return obj_district[key_district]
      })
...
export function cutAddress(address: string) {
  ...
  const addr = array.filter((v, index) => {
    if (index === (length - 3)) {
      return v
    }
